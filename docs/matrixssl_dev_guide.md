...
In _MatrixSSL Commercial Edition_, Certificate Authority root and child certificates can be created using the provided command-line tools and API. For more information, please consult the _Matrix Key and Cert Generation Utilities_ and _MatrixSSL Certificates and Certificate Revocation Lists_ documents.
...
MatrixSSL ships with a few example configurations, which are described here. _MatrixSSL FIPS edition_ contains additional example configurations; these are described in the _MatrixCrypto CL_ document included in that distribution.
